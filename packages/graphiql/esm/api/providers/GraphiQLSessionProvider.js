...
import { SessionActionTypes, operationRequestAction, operationSucceededAction, variableChangedAction, operationChangedAction, operationErroredAction, tabChangedAction, } from '../actions/sessionActions';
...
export var initialContextState = __assign({ executeOperation: function () { return __awaiter(void 0, void 0, void 0, function () { return __generator(this, function (_a) {
        return [2];
    }); }); }, changeOperation: function () { return null; }, changeVariables: function () { return null; }, changeTab: function () { return null; }, operationError: function () { return null; }, dispatch: function () { return null; } }, initialState);
export var SessionContext = React.createContext(initialContextState);
export var useSessionContext = function () { return React.useContext(SessionContext); };
...
export function SessionProvider(_a) {
    ...
    var schemaState = React.useContext(SchemaContext);
    var editorsState = React.useContext(EditorContext);
    var _b = React.useReducer(sessionReducer, initialState), state = _b[0], dispatch = _b[1];
    var operationError = React.useCallback(function (error) { return dispatch(operationErroredAction(error, sessionId)); }, [dispatch, sessionId]);
    var changeOperation = React.useCallback(function (operationText) {
        return dispatch(operationChangedAction(operationText, sessionId));
    }, [dispatch, sessionId]);
    var changeVariables = React.useCallback(function (variablesText) {
        return dispatch(variableChangedAction(variablesText, sessionId));
    }, [dispatch, sessionId]);
    var changeTab = React.useCallback(function (pane, tabId) { return dispatch(tabChangedAction(pane, tabId)); }, [dispatch]);
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    ...
                    return [4, observableToPromise(fetcher(fetchValues))];
                case 1:
                    result = _b.sent();
    ...
    return (React.createElement(SessionContext.Provider, { value: __assign(__assign(__assign({}, state), session), { executeOperation: executeOperation,
